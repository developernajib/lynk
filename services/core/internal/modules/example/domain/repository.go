...
//
// ownerID is an explicit parameter on reads so ownership scoping can never
// be forgotten silently.
type NoteRepository interface {
	// Create inserts a new note.
	Create(ctx context.Context, note *Note) error
	// Get loads one of the owner's notes, or ErrNoteNotFound.
	Get(ctx context.Context, ownerID string, id vo.NoteID) (*Note, error)
	...
	List(ctx context.Context, ownerID string, limit, offset int32) ([]*Note, error)
}
